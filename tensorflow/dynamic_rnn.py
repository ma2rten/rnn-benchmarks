...
x = tf.placeholder(tf.float32, [batch_size, seq_length, input_size], name="x")
...
final = output[:, seq_length-1, :]
cost = tf.reduce_mean((final - y) ** 2)
...
for k, i in enumerate(xrange(0, n_samples, batch_size)):
    session.run(final, feed_dict=get_feed_dict(X_data[i:i+batch_size, :, :])) 
